...
from medusa_analyzer.backend.filters import FilterResponse
...
class FilterPreviewPlot(QFrame):
    ...
    def _frequency_ticks(
        cls, maximum_frequency: float, plot_width: float
    ...
    def paintEvent(self, event):
        ...
        maximum_frequency = (
            max(self.response.frequencies)
            if self.response and self.response.frequencies
            else 1.0
        )
        axis_maximum, frequency_ticks = self._frequency_ticks(
            maximum_frequency, plot.width()
        )
        ...
        for frequency in frequency_ticks:
            ...
            label_width = 58.0
            label_left = max(
                1.0, min(self.width() - label_width - 1.0, x - label_width / 2)
            )
            painter.drawText(
