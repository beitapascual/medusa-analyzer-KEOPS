...
class PSDPlot(BasePlot):
    ...
    def load_data(self, filtered_files: dict[str, list[str]], selected_channels: list[int]) -> None:
            ...
            for filepath in file_list:
                ...
                if not np.array_equal(freqs, freqs_ref):
                    try:
                        ...
                        print(f"[WARN] Interpolation failed for {filepath}: {error}")
                        continue
        ...
        if groups:
            ...
            self._psd_data = {group[0]: {"mean": group[2], "std": group[4], "n": group[5]} for group in groups}
            self._psd_data_stats = {group[0]: group[3] for group in groups}
    ...
    def draw(self, colors: dict[str, str] | None = None) -> None:
        ...
        for group_name, data in self._psd_data.items():
            ...
            self.ax.plot(self._freqs, mean_psd, label=group_name, color=color, linestyle=line_style,
                linewidth=line_width, alpha=0.9)

            if plot_error and n_subjects > 1:
                ci = 1.96 * (std_psd / np.sqrt(n_subjects))
                self.ax.fill_between(self._freqs, mean_psd - ci, mean_psd + ci, color=color, alpha=0.25)
        ...
        for start, end, _, color in bands:
            self.ax.axvspan(start, end if end is not None else x_max, color=color, alpha=0.2,
        ...
        if stats_checkbox and current_tab is not None and hasattr(self.tabs_widget, "controller"):
            ...
            self.ax.fill_between(self._freqs, 0, 1, where=(p_vals < 0.05), facecolor="gray",
                edgecolor="black", linewidth=1.5, alpha=0.1, transform=self.ax.get_xaxis_transform(),
                zorder=0, label="p < 0.05")

        # Limites y estilo desde BasePlot
