...
class FilterControls(QFrame):
    ...
    def __init__(self, title: str, config: FilterConfig):
        ...
        self.window.addItems(["hamming", "barthann", "bartlett", "blackman", "blackmanharris", "bohman", "boxcar",
        ...
        self.iir_rp.setValue(config.iir_rp_db)
        self.iir_rp.setSuffix(" dB")
        self.iir_rs = QDoubleSpinBox()
        self.iir_rs.setRange(1.0, 200.0)
        self.iir_rs.setDecimals(1)
        self.iir_rs.setValue(config.iir_rs_db)
        self.iir_rs.setSuffix(" dB")
        self.design = QComboBox()
        self.design.addItem("Butterworth", "butter")
        self.design.addItem("Chebyshev I", "cheby1")
        self.design.addItem("Chebyshev II", "cheby2")
        self.design.addItem("Elliptic", "ellip")
        self.design.addItem("Bessel", "bessel")
    ...
    def _sync(self):
        ...
        needs_rp = self.config.iir_design in {"cheby1", "cheby2"}
        needs_rs = self.config.iir_design in {"cheby1", "cheby2"}
        self.iir_rp.setEnabled(self.config.enabled and self.config.filter_type == "iir" and needs_rp)
        self.iir_rs.setEnabled(self.config.enabled and self.config.filter_type == "iir" and needs_rs)
