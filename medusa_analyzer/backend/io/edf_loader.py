...
def _read_repeated_strings(payload: bytes, offset: int, count: int, width: int) -> tuple[list[str], int]:
    values = [
        _read_ascii(payload[offset + index * width: offset + (index + 1) * width])
...
def _read_repeated_ints(payload: bytes, offset: int, count: int, width: int) -> tuple[list[int], int]:
    values = [
        _read_int(payload[offset + index * width: offset + (index + 1) * width])
        for index in range(count)
    ]
    return values, offset + count * width
...
def _read_repeated_floats(payload: bytes, offset: int, count: int, width: int) -> tuple[list[float | None], int]:
    values = [
        _read_float(payload[offset + index * width: offset + (index + 1) * width])
        for index in range(count)
    ]
    return values, offset + count * width
...
def load_edf_file(
    ...
    sampling_rates = [
        (samples / record_duration) if record_duration and samples else None
        for samples in samples_per_record
    ]

    channel_arrays = [
        np.empty(total_samples, dtype=np.float64)
        for total_samples in total_samples_per_channel
    ]
